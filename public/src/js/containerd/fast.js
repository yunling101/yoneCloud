var Fast = {
    ...
    view: function(){
        ...
        var r = Fast.router();
        //验证授权
        if (require.config.auth == "0" && r.href != "/system/config") {
           r.href = "/system/authorize"
           r.path = ["system", "Authorize"]
        }
    ...
    // username: function(){
    //    var result = "";
    //    Fast.api.ajax({type: "GET", url: "/users/", dataType: "json", async: false}, function(d) {
    //         result =  d;
    //    });
    //    return result;
    //},
    ...
    oAuth: function(){
        Fast.api.ajax({url: Config.oauth, type: "GET", dataType: "json"}, function(d){
            if(d.code) {
                var line_domain = document.domain.split(".");
                var domain = line_domain.slice(1).join(".");
                document.cookie="oauth_grafana=" +d.msg.oauth_state+ "; path=/; domain=." + domain;
                return true;
            }else{
                console.log(d);
                return false;
