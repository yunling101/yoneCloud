        ...
        table.bootstrapTable({
            ...
            columns: [
                [
                    ...
                    {field: 'is_finished', title: "连接" + L('Status'), formatter: Controller.api.formatter.finished},
    ...
    api:{
        formatter: {
            finished: function(value, row, index) {
                if (value){
                    var m = '<span class="text-danger">断开</span>';
                }else{
                    var m = '<span class="text-info">在线</span>';
                }
                return m;
            }
        },
            ...
            connect: function(url){
                ...
                Fast.api.ajax({"url": url, "dataType":"json"}, function(data) {
                    if (data.code){
                       ...
                       body += '用户名: <span class="text-info">'+ v.username +'</span>';
                       body += ' 服务器: <span class="text-info">'+ v.hosts +'</span>';
                       body += ' 开始时间: <span class="text-info">'+ v.start_time +'</span></div>';
