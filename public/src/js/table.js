...
var Table = {
    ...
    config: {
        ...
        markread: '.btn-markread',
        markreadall: '.btn-markread-all',
        dragsortfield: 'weigh',
        inputupload: "#toolbar-upload",
    },
    button: {
        edit: {
            name: 'edit',
    ...
    api: {
        ...
        bindevent: function (table) {
            ...
            table.on('load-error.bs.table', function (status, res, e) {
                ...
                Toastr.error(L('Unknown data format'));
            });
            //当加载数据成功时
            ...
            $(toolbar).on('click', Table.config.addbtn, function () {
                ...
                if (options.callback && typeof options.callback.add === 'function') {
                    $(this).data().callback = options.callback.add();
                }
                if (options.callback && typeof options.callback.html === 'function') {
                    $(this).data().html = options.callback.html();
                }
                if (options.extend.form) {
                    $(this).data().form = options.extend.form;
                }

                Fast.api.open(url, L('Add'), $(this).data() || {});
            });
            // 标记为已读
            $(toolbar).on('click', Table.config.markread, function () {
                var ids = Table.api.selectedids(table);
                var url = options.extend.multi_url;
                Table.api.multi(url, ids, table, this);
            })
            // 全标记为已读
            $(toolbar).on('click', Table.config.markreadall, function () {
                var ids = Table.api.selectedids(table);
                var url = options.extend.multi_url;

                var that = this;
                $(that).data().params = "all";
                Layer.confirm(
                    L('Are you sure you want all the markers read?'),
                    { icon: 3, title: L('Warning'), shadeClose: true },
                    function (index) {
                        Table.api.multi(url, ids, table, that);
                        Layer.close(index);
                    }
                );
            })
            ...
            $(toolbar).on('click', Table.config.importbtn, function () {
                ...
                $(Table.config.inputupload).click() // $(selection).trigger("click")

                try {
                    $(toolbar).on("change", Table.config.inputupload, function () {
                        ...
                        Fast.api.ajax({
                            ...
                            "contentType": false,
                            "beforeSend": function () {
                                layer_ids = layer.open({ type: 3, icon: 2 })
                            },
                            "complete": function () {
                                layer.close(layer_ids);
                            }
                        }, function (res) {
                            if (res.code) {
                                table.bootstrapTable('refresh');
                        ...
                        $(Table.config.inputupload).remove();
                        $(toolbar).append('<input class="hide" type="file" id="toolbar-upload"/>');
                    });
                } catch (e) {
                    Toastr.error("前端异常错误, 请刷新后重试！");
                    return false;
                }
            });
            // 批量编辑按钮事件
            $(toolbar).on('click', Table.config.editbtn, function () {
                ...
                $.each(table.bootstrapTable('getSelections'), function (index, row) {
                    ...
                    Fast.api.open(url, L('Edit'), $(that).data() || {});
                });
            });
            //清空回收站
            $(document).on('click', Table.config.destroyallbtn, function () {
                ...
                Layer.confirm(L('Are you sure you want to truncate?'), function () {
                    var url = $(that).data("url") ? $(that).data("url") : $(that).attr("href");
                    Fast.api.ajax({ "url": url, type: "POST" }, function () {
                        Layer.closeAll();
                        table.bootstrapTable('refresh');
                    }, function () {
                        Layer.closeAll();
            ...
            $(document).on('click', Table.config.restoreallbtn + ',' + Table.config.restoreonebtn + ',' + Table.config.destroyonebtn, function () {
                var that = this;
                var url = $(that).data("url") ? $(that).data("url") : $(that).attr("href");
                Fast.api.ajax({ "url": url, type: "POST", "data": { "ids": "all" } }, function () {
                    table.bootstrapTable('refresh');
                });
                return false;
            });
            ...
            $(toolbar).on('click', Table.config.delbtn, function () {
                ...
                var ids = Table.api.selectedids(table);
                Layer.confirm(
                    L('Are you sure you want to delete the %s selected item?', ids.length),
                    { icon: 3, title: L('Warning'), shadeClose: true },// 原有offset: 0,
                    function (index) {
            ...
            $(table).on("click", "[data-id].btn-edit", function (e) {
                ...
                Fast.api.open(url, L('Edit'), $(this).data() || {});
            });
        ...
        multi: function (action, ids, table, element) {
            ...
            var options = { url: url, type: "POST", dataType: "json", data: { ids: ids, params: params } }
        ...
        events: {
            operate: {
                'click .btn-editone': function (e, value, row, index) {
                    ...
                    if (options.callback && typeof options.callback.edit === 'function') {
                        $(this).data().callback = options.callback.edit(row);
                    }
                    if (options.callback && typeof options.callback.html === 'function') {
                        $(this).data().html = options.callback.html(row);
                    }
                    if (options.extend.form) {
                        $(this).data().form = options.extend.form;
                    }

                    Fast.api.open(options.extend.edit_url, L('Edit'), $(this).data() || {});
                },
                'click .btn-delone': function (e, value, row, index) {
                    ...
                    Layer.confirm(
                        L('Are you sure you want to delete this item?'),
                        { icon: 3, title: L('Warning'), offset: [top, left], shadeClose: true },
                        function (index) {
                            var table = $(that).closest('table');
                            var options = table.bootstrapTable('getOptions');
                            if (options.callback && typeof options.callback.del === 'function') {
                                $(that).data().params = options.callback.del(row);
                            }
        ...
        formatter: {
            ...
            normal: function (value, row, index) {
                ...
                if (!display) {
                    display = L(value.charAt(0).toUpperCase() + value.slice(1));
                }
                var html = '<span class="text-' + color + '">' + (icon ? '<i class="' + icon + '"></i> ' : '') + display + '</span>';
                if (this.operate != false) {
                    html = '<a href="javascript:;" class="searchit" data-toggle="tooltip" title="' + L('Click to search %s', display) + '" data-field="' + this.field + '" data-value="' + value + '">' + html + '</a>';
                }
                return html;
            },
            toggle: function (value, row, index) {
                ...
                return "<a href='javascript:;' data-toggle='tooltip' title='" + L('Click to toggle') + "' class='btn-change " + (disable ? 'btn disabled' : '') + "' data-id='"
            ...
            search: function (value, row, index) {
                ...
                return '<a href="javascript:;" class="searchit" data-toggle="tooltip" title="' + L('Click to search %s', value) + '" data-field="' + field + '" data-value="' + value + '">' + value + '</a>';
            },
            addtabs: function (value, row, index) {
                var url = Table.api.replaceurl(this.url, row, this.table);
                var title = this.atitle ? this.atitle : L("Search %s", value);
                return '<a href="' + Fast.api.fixurl(url) + '" class="addtabsit" data-value="' + value + '" title="' + title + '">' + value + '</a>';
            },
            dialog: function (value, row, index) {
                var url = Table.api.replaceurl(this.url, row, this.table);
                var title = this.atitle ? this.atitle : L("View %s", value);
            ...
            flag: function (value, row, index) {
                ...
                $.each(arr, function (i, value) {
                    ...
                    display = typeof that.searchList !== 'undefined' && typeof that.searchList[value] !== 'undefined' ? that.searchList[value] : L(value.charAt(0).toUpperCase() + value.slice(1));
                    label = '<span class="label label-' + color + '">' + display + '</span>';
                    if (that.operate) {
                        html.push('<a href="javascript:;" class="searchit" data-toggle="tooltip" title="' + L('Click to search %s', display) + '" data-field="' + field + '" data-value="' + value + '">' + label + '</a>');
                    } else {
                        html.push(label);
                    }
                });
            ...
            datetime: function (value, row, index) {
                ...
                if (isNaN(value)) {
                    return value ? moment(value).format(datetimeFormat) : L('None');
                } else {
                    return value ? moment(parseInt(value) * 1000).format(datetimeFormat) : L('None');
                }
            },
            operate: function (value, row, index, disable) {
                var table = this.table;
                // 操作配置
                var options = table ? table.bootstrapTable('getOptions') : {};
                // 默认按钮组
                ...
                if (this.callback != undefined && typeof (this.callback) == 'function') {
                    this.callback(value, row, index, buttons)
                }
                // 所有按钮名称
                var names = [];
                ...
                if (options.extend.del_url !== '' && names.indexOf('del') === -1 && !disable) {
        ...
        buttonlink: function (column, buttons, value, row, index, type) {
            ...
            if (!$.isEmptyObject(dropdowns)) {
                ...
                $.each(dropdowns, function (i, j) {
                    dropdownHtml.push('<div class="btn-group"><button type="button" class="btn btn-white dropdown-toggle btn-xs" data-toggle="dropdown">' + i + '</button><button type="button" class="btn btn-white dropdown-toggle btn-xs" data-toggle="dropdown"><span class="caret"></span></button><ul class="dropdown-menu pull-right"><li>' + j.join('</li><li>') + '</li></ul></div>');
                });
                html.push(dropdownHtml);
